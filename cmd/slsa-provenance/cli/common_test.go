...
var (
	githubContextEnv = map[string]string{
		"GITHUB_REPOSITORY": "philips-labs/slsa-provenance-action",
		"GITHUB_RUN_ID":     "1332651620",
		"GITHUB_WORKFLOW":   "Integration test file provenance",
		"GITHUB_SHA":        "c4f679f131dfb7f810fd411ac9475549d1c393df",
	}
)

func TestMain(m *testing.M) {
	tmpFile, err := os.CreateTemp("", "event")
	if err != nil {
		fmt.Printf("error creating event path: %v\n", err)
		os.Exit(1)
	}
	defer os.Remove(tmpFile.Name())
	if _, err := tmpFile.Write([]byte(githubContextEvent)); err != nil {
		fmt.Printf("error writing to event path: %v\n", err)
		os.Exit(1)
	}
	os.Setenv("GITHUB_EVENT_PATH", tmpFile.Name())
	defer os.Unsetenv("GITHUB_EVENT_PATH")
	for k, v := range githubContextEnv {
		os.Setenv(k, v)
		defer os.Unsetenv(k)
	}
	m.Run()
